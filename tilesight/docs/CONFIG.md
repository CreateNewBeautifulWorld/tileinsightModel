...
| `memory.dma.hugepage_KB` | float | KB | `0` | calib | Fixed size of one DMA operation: a DMA moves exactly one mega tile, never less (0 = unset: fall back to an occupancy proxy for L2/DDR slice spread). Must be a whole multiple of memory.addressing.l2's granularity x port count, so a mega tile always splits evenly across every memory slice by construction; a transfer smaller than one mega tile still pays for the whole thing (rounding waste), which is the trade-off against setting it too large |
