...
DB_DIR = Path(__file__).parent.parent.parent / "gpuPresets"
        ...
        for k in path.split("."):
            if not isinstance(node, dict) or k not in node:
                if default is not HardwareSpec._MISSING:
                    return default
                from tilesight.gpuTilingPerfHWModel.interfaceAndRun.schema import default_for
                return default_for(path)
            node = node[k]
        return node

    def validate(self) -> list[str]:
        from tilesight.gpuTilingPerfHWModel.interfaceAndRun.schema import validate
