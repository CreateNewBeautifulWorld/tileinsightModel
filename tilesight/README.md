...
- Single-GPU mode: pick the card (b300 / b200 / h200, plus dotted overrides), pick a kernel
  ...
  `python -m tilesight.cli.cli kernel --gpu-tiling-perf-hw-model b300 --kernel gemm --dtype fp8 --shape '{"M":4096,"N":4096,"K":7168}' --unit cyc --trace-out trace.txt --csv-out cycles.csv --xlsx-out cycles.xlsx`
...
1. **GPU config** — `gpuTilingPerfHWModel/interfaceAndRun/schema.py`, 99 fields. `tilesight config --list`.
1b. **GPU config, slice form** — `gpuTilingPerfHWModel/interfaceAndRun/slice_config.py`, 42 fields: shader slices (cores per slice,
...
A GPU is input, not model. `gpuTilingPerfHWModel/interfaceAndRun/schema.py` declares all 99 config fields (15 sections; tags:
...
## Inputs
- **Model** (`gpuTilingPerfHWModel/interfaceAndRun/model_spec.py`): block-level YAML (`mha`, `gqa`/MQA, `mla`, `mlp`, `moe`,
...
- **Hardware** (`gpuPresets/*.yaml`): NVIDIA **B300** (default), **B200**, **H200**; AMD **MI300X**,
