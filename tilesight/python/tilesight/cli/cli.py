"""CLI.
  ...
  python -m tilesight.cli.cli run   --model kimi_k2.hf --gpu-tiling-perf-hw-model b300 --phase decode --batch 256 --seq 8192 --dp 8
  python -m tilesight.cli.cli request --model kimi_k2.hf --gpu-tiling-perf-hw-model b300 --batch 256 --dp 8 --prompt 8192 --output 4096
...
def main(argv=None):
    ...
    if a.cmd == "gpu":
        ...
        return
    if a.cmd == "config":
        from tilesight.gpuTilingPerfHWModel.interfaceAndRun.schema import FIELDS, SECTIONS, as_csv, as_markdown, _Required
        if a.validate:
        ...
        if a.workload:
            from tilesight.gpuTilingPerfHWModel.interfaceAndRun.workload import WORKLOAD_FIELDS, W_SECTIONS, as_markdown as w_md
            if a.md:
    ...
    if a.cmd == "buffer":
        ...
        from tilesight.gpuTilingPerfHWModel.model.dse.sweep import rows_to_csv
        ...
        return
    if a.cmd == "kernel":
        from tilesight.gpuTilingPerfHWModel.genResult.timeline import cycle_csv, machine_timeline, steady_timeline, timeline_text, trace_text
