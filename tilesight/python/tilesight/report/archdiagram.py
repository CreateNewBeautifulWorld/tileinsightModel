...
def arch_svg(hw: HardwareSpec, width: int = 860) -> str:
    g = []
    sms = hw.sms
    l1 = hw.get("memory.l1") or {}
    smem_kb = hw.get("memory.onchip.smem.capacity_KB")
    tmem_kb = hw.get("memory.onchip.tmem.capacity_KB") or 0
    parts = int(hw.get("memory.l2.partitions") or 1)
    blocks = int(hw.get("memory.l2.blocks") or 0)
    ports = int(hw.get("memory.ddr.ports") or 1)
    engines = int(hw.get("memory.dma.engines") or 1)
    sram = hw.sram
    ...
    for i in range(shown):
        ...
        if tmem_kb:
            ...
            g.append(_box(x + 8, y + 50, bw - 16, 20, C_L1,
                          f"L1 {hw.l1_capacity_bytes / 1024:.0f} KB", "", rx=4, fs=9))
    g.append(f'<text x="{width / 2}" y="{y - 12}" font-size="12" text-anchor="middle" '
             f'fill="currentColor" opacity=".75">{sms} SMs @ {hw.clock_hz / 1e9:.2f} GHz'
             + (f" · clusters of {cluster} share L1 via DSMEM" if cluster > 1 else "") + '</text>')

    # --- load paths
    y2 = y + 96
    paths = list((hw.get("load_paths") or {}).keys())
    pw = min(170, (width - 60) / max(1, len(paths)) - 10)
    for i, p in enumerate(paths):
        px = 30 + i * (pw + 10)
        kind = "DMA engine" if hw.path_is_dma(p) else "vector loads"
        g.append(_box(px, y2, pw, 34, C_DMA, f"{p}", f"{kind} · {hw.path_attr(p, 'per_sm_GBps', 0)} GB/s/SM"))
    g.append(f'<text x="{width - 30}" y="{y2 + 20}" font-size="10" text-anchor="end" fill="currentColor" '
             f'opacity=".7">{engines} DMA engine(s), {"per L2 block" if hw.get("memory.dma.per_l2_block") else "shared"}'
             f' → {hw.dma_destination}</text>')
    ...
    if sram:
        ...
        y3 += 32
        pieces = min(6, max(1, int(hw.sms // max(1, int((hw.get("memory.l1") or {}).get("cluster_size", 1))))))
    ...
    for i in range(parts):
        g.append(_box(30 + i * (pwid + 10), y3, pwid, 46, C_L2, f"L2 partition {i}",
                      f"{hw.l2_capacity_bytes / parts / 1024 / 1024:.0f} MiB"))
    sub = f"L2 {hw.get('memory.l2.capacity_MB'):.0f} MB · {hw.get('memory.l2.bandwidth_TBps')} TB/s"
    if blocks:
        sub += f" · {blocks} block(s) x {hw.get('memory.l2.ports_per_block')} port(s)"
    sub += f" · {hw.get('memory.addressing.l2', {}).get('mode', 'interleave')} @ " \
           f"{hw.get('memory.addressing.l2', {}).get('granularity_KB', 1)} KB"
    ...
    g.append(f'<text x="{width / 2}" y="{y4 + 50}" font-size="11" text-anchor="middle" fill="currentColor">'
             f'HBM {hw.get("memory.ddr.capacity_GB"):.0f} GB · {hw.get("memory.ddr.bandwidth_TBps")} TB/s · '
             f'{ports} ports · {hw.get("memory.ddr.latency_ns")} ns</text>')

    # --- scale-up link
    y5 = y4 + 64
    nv = hw.get("network.nvlink") or {}
    ...
    return (f'<svg viewBox="0 0 {width} {h}" width="100%" role="img">'
            ...
            f'{html.escape(hw.name)} — as configured</text>' + "".join(g) + "</svg>")
