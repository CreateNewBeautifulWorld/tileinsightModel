"""Design-space exploration: vary hardware fields, re-run the model, find requirements.
    ...
    sweep(model, hw, rc, "memory.ddr.bandwidth_TBps", [4, 6, 8, 12, 16])
    required_value(model, hw, rc, "memory.ddr.bandwidth_TBps", target_step_ms=15, lo=2, hi=64)
...
def default_links(hw: HardwareSpec, path: str):
    ...
    if path != "sms":
        return None
    base_sms = hw.sms
    tc = dict(hw.get("compute.tc_dense_tflops") or {})
    cuda = hw.get("compute.cuda_fp32_tflops")
    sfu = hw.get("compute.sfu_tops")
...
def sweep(model: ModelSpec, hw: HardwareSpec, rc: RunConfig, path: str, values: list[float],
          linked: Callable[[float], dict] | None = None, progress=None, auto_link: bool = True):
    if linked is None and auto_link:
        linked = default_links(hw, path)
    rows = []
...
def required_value(model: ModelSpec, hw: HardwareSpec, rc: RunConfig, path: str, target_step_ms: float,
    ...
    def t(v):
        ...
        if linked:
            ch.update(linked(v))
        return run_model(model, hw.override(ch), rc).step_time_s * 1e3
