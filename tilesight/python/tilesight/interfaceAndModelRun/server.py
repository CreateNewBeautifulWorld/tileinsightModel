"""Local web server: your machine computes, others just open the page.
    ...
    python -m tilesight.interfaceAndModelRun.cli serve --host 0.0.0.0 --port 8000
...
from tilesight.model.dse.sweep import required_value, sweep
...
from tilesight.model.kernels.gemm import lower_elementwise, lower_gemm
from tilesight.model.kernels.tiles import AttnTileConfig, TileConfig, attn_search_space, gemm_search_space
from tilesight.interfaceAndModelRun.gpuTilingPerfHWModel.spec import DB_DIR, HardwareSpec
...
from tilesight.interfaceAndModelRun.runner import CurModelConfig, run
from tilesight.interfaceAndModelRun.spec import PRESET_DIR, ModelSpec
from tilesight.interfaceAndModelRun.workload import WORKLOAD_FIELDS, W_SECTIONS, run_workload, validate_workload
from tilesight.interfaceAndModelRun.gpuTilingPerfHWModel.slice_config import SLICE_FIELDS, derive as slice_derive, to_hardware_spec, validate_slice_config
from tilesight.interfaceAndModelRun.workload import compare_attention_impl, memory_breakdown
from tilesight.generateResult.report.archdiagram import arch_svg
from tilesight.generateResult.report.table import bound_report, model_summary, resource_class
from tilesight.generateResult.report.timeline import cycle_csv, machine_timeline, steady_timeline, trace_text
from tilesight.interfaceAndModelRun.request import request_summary
...
def _top_kernels(rep, cur_gpu_config, n: int = 3):
    ...
    from tilesight.interfaceAndModelRun.runner import resolve_op
    out = []
    for o in sorted(rep.ops, key=lambda o: -o.total_s):
        ...
        try:
            ...
            from tilesight.model.kernels.tiles import AttnTileConfig, TileConfig
            p = o.op.p
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):                                       # noqa: N802
        ...
        if path == "/api/schema":
            from tilesight.interfaceAndModelRun.gpuTilingPerfHWModel.schema import FIELDS as HW_FIELDS
        ...
        if path == "/api/pdf":
            import tempfile
            from tilesight.generateResult.report.pdfreport import write_pdf
            q = dict(p.split("=", 1) for p in self.path.split("?", 1)[-1].split("&") if "=" in p)
            with _LOCK:
                job = _JOBS.get(q.get("job", ""))
            res = (job or {}).get("result") or {}
            ...
            try:
                ...
                from tilesight.interfaceAndModelRun.runner import resolve_op  # noqa: F401
    ...
    def do_POST(self):                                      # noqa: N802
        ...
        if self.path.split("?")[0] == "/api/validate_gpu_tiling_perf_hw_model":
            ...
            from tilesight.interfaceAndModelRun.gpuTilingPerfHWModel.schema import validate as _v
...
def serve(host: str = "127.0.0.1", port: int = 8000) -> None:
    from tilesight.model.engine.backend import BACKEND
