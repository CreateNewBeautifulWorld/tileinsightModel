"""CLI.
  ...
  python -m tilesight.interfaceAndModelRun.cli run   --model kimi_k2.hf --gpu-tiling-perf-hw-model b300 --phase decode --batch 256 --seq 8192 --dp 8
  python -m tilesight.interfaceAndModelRun.cli request --model kimi_k2.hf --gpu-tiling-perf-hw-model b300 --batch 256 --dp 8 --prompt 8192 --output 4096
  python -m tilesight.interfaceAndModelRun.cli sweep --model kimi_k2.hf --gpu-tiling-perf-hw-model b300 --param memory.ddr.bandwidth_TBps --values 4,6,8,12,16
  python -m tilesight.interfaceAndModelRun.cli need  --model kimi_k2.hf --gpu-tiling-perf-hw-model b300 --param memory.ddr.bandwidth_TBps --target-ms 20
  python -m tilesight.interfaceAndModelRun.cli dump-model --model kimi_k2.hf      # editable block-level YAML
  python -m tilesight.interfaceAndModelRun.cli serve --host 0.0.0.0 --port 8000   # web UI; computation stays on this machine
"""
...
from tilesight.generateResult.report.table import model_summary, ops_csv
...
def main(argv=None):
    ...
    bp.add_argument("--gpu-tiling-perf-hw-model", dest="cur_gpu_config", default="b300")
    ...
    kp.add_argument("--gpu-tiling-perf-hw-model", dest="cur_gpu_config", default="b300")
    ...
    mp.add_argument("--gpu-tiling-perf-hw-model", dest="cur_gpu_config", default="b300")
    ...
    for name in ("run", "request", "sweep", "need", "dump-model"):
        ...
        p.add_argument("--gpu-tiling-perf-hw-model", dest="cur_gpu_config", default="b300")
    ...
    if a.cmd == "serve":
        from tilesight.interfaceAndModelRun.server import serve
        return serve(a.host, a.port)
    if a.cmd == "gpu":
        from tilesight.interfaceAndModelRun.gpuTilingPerfHWModel.slice_config import (SLICE_FIELDS, as_markdown as s_md, derive,
        ...
        return
    if a.cmd == "config":
        from tilesight.interfaceAndModelRun.gpuTilingPerfHWModel.schema import FIELDS, SECTIONS, as_csv, as_markdown, _Required
        if a.validate:
        ...
        if a.workload:
            from tilesight.interfaceAndModelRun.workload import WORKLOAD_FIELDS, W_SECTIONS, as_markdown as w_md
            if a.md:
    ...
    if a.cmd == "memmap":
        ...
        return
    if a.cmd == "buffer":
        from tilesight.model.dse.buffer import best_alloc, capacity_curve, optimize_buffer, profile_classes, with_buffer
        from tilesight.model.dse.sweep import rows_to_csv
        ...
        return
    if a.cmd == "kernel":
        from tilesight.generateResult.report.timeline import cycle_csv, machine_timeline, steady_timeline, timeline_text, trace_text
        ...
        if a.kernel in ("gemm", "grouped_gemm"):
            from tilesight.interfaceAndModelRun.gpuTilingPerfHWModel.spec import DTYPE_BYTES as _DT
            from tilesight.model.kernels.tiles import TileConfig as _TC
    ...
    # tiles) and cur_model_config (what runs on it). --model/--gpu-tiling-perf-hw-model/--run-config/--set/--tile just
    ...
    if a.cmd == "request":
        from tilesight.interfaceAndModelRun.request import request_summary, run_request
