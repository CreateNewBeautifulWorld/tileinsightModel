...
from tilesight.gpuTilingPerfHWModel.model.kernels.gemm import gload, gstore, lower_elementwise, occupancy, reg_estimate, resident_frac, tensor_class
from tilesight.gpuTilingPerfHWModel.model.kernels.tiles import AttnTileConfig
...
def lower_attention_decode(cur_gpu_config: HardwareSpec, name: str, *, B: int, H: int, kv_heads: int, S: int,
    ...
    for it in range(min(iters, 4)):
        for b, h, g, s in order:
            ...
            streams.append(0)
    from tilesight.gpuTilingPerfHWModel.model.engine.cache_sim import simulate as _sim
