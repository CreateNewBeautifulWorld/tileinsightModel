...
from tilesight.model.workload import (WORKLOAD_FIELDS, run_workload, to_run_config,
                                      validate_workload)
from tilesight.report.archdiagram import arch_svg
from tilesight.server import Handler
...
def test_pdf_report(tmp_path):
    ...
    from tilesight.report.pdfreport import unit_groups, write_pdf
...
def test_slice_config_derives_and_translates():
    import yaml
    from tilesight.gpuTilingPerfHWModel.slice_config import (derive, to_hardware_spec, validate_slice_config)
...
def test_kimi_k3_preset_memory_matches_the_simulation():
    import yaml
    from tilesight.model.workload import memory_breakdown
    cfg = yaml.safe_load(open("python/tilesight/model/presets/kimi_k3_10L.yaml"))
    assert validate_workload(cfg) == []
...
def test_derive_endpoint_and_slice_job(url):
    import yaml
    cfg = yaml.safe_load(open("examples/slice_gpu.yaml"))
    wl = yaml.safe_load(open("python/tilesight/model/presets/kimi_k3_10L.yaml"))
...
def test_model_is_organised_in_three_blocks():
    from tilesight.gpuTilingPerfHWModel.spec import lane_domain
...
def test_flash_attention_is_a_choice_with_consequences():
    from tilesight.model.workload import compare_attention_impl
