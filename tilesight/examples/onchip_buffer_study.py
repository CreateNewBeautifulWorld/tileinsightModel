...
hw, model = HardwareSpec.load("b300"), ModelSpec.load("kimi_k2.hf")
rc = RunConfig(phase="decode", batch=256, seq_len=8192, dp=8)
L2 = hw.get("memory.l2.capacity_MB")

print(f"# capacity curve (L2 = {L2} MB), buffer bandwidth = L2 bandwidth, bypass+prefetch on")
for r in capacity_curve(model, hw, rc, (0, L2, 2 * L2, 4 * L2, 8 * L2, 16 * L2),
...
best, rows = optimize_buffer(model, hw, rc, cap, steps=2)
for r in rows[:8]:
...
for bw in (5, 10, 20, 40):
    h = with_buffer(hw, 8 * L2, bandwidth_TBps=bw, bypass_l2=True, prefetch=True, costream=True)
...
for cap_mb in (0, 4 * L2):
    h = with_buffer(hw, cap_mb, bypass_l2=True, prefetch=True, costream=True)
